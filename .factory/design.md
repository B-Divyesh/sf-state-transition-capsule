...
- `site/public/apple-touch-icon.png`: a deterministic 180×180 PNG rasterization of the hand-made `favicon.svg` signal trace. Created locally with the pinned Playwright Chromium renderer because the worker image has no SVG raster delegate; it is a distinct Apple touch asset, uses the existing original icon geometry, and introduces no external image or license.
